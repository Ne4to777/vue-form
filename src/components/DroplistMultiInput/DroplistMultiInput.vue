  ...
  computed: {
    ...
    menuContentItemsMapped() {
      return this.menuItems.reduce((acc, el, i) => {
        acc[
          el.hasOwnProperty(this.keyProperty) ? el[this.keyProperty] : el
        ] = i;
        return acc;
      }, {});
    },
    placeholderComputed() {
      return this.limit && this.limit <= this.activeIndexes.length
        ? MESSAGE.disabled
        : this.placeholder;
    }
  },
